...
function CookTable({ cooks, handleRemove, removeData, time, calcium }) {
    ...
    return (
        <div className='p-4 border lg:w-1/2 rounded-lg'>
                    ...
                    {cooks.map((data, index) => {
                        return (
                            <tr key={index} className=' shadow-sm *:font-normal border p-3'>
                                ...
                                <th>{data.calories}</th>
                                <th className='btn btn-primary' onClick={() => handleRemove(data)}>Preparing</th>
                            </tr>
                        )
                    })}
                </tbody>
            ...
            <div className='mt-24'>
                <table className='w-full'>
                    <caption className='border-b-2 p-4 font-black'>Currently cooking: {removeData.length < 10 ? "0" + removeData.length : removeData.length}</caption>
                    <thead>
                        <tr className='*:font-normal'>
                            <th></th>
                            <th>Name</th>
